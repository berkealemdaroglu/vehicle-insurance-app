...
class VehicleInsuranceFilter(private val vehicleInsuranceResponse: VehicleInsuranceResponse?) {
    ...
    fun filterByBrand(years: String?, brands: ((List<Brand>?) -> Unit)) {
        CoroutineScope(Dispatchers.Default).launch {
            val filterBrands =
                vehicleInsuranceResponse?.years?.firstOrNull { it?.year == years?.toIntOrNull() }?.brands?.map { it.copy(vehicleYear = years) }
            withContext(Dispatchers.Main) {
                brands(filterBrands)
            }
        }
                    ...
                    ?.flatMap { brand ->
                        brand.vehicleModels?.map { model ->
                            Brand(
                                ...
                                vehicleModels = listOf(VehicleModel(model?.price, model?.modelName))
                            )
                        } ?: listOf()
                    } ?: listOf()
